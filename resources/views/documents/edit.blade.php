            ...
            <form action="{{ route('documents.update', $document) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('afdeling_id') is-invalid @enderror" id="afdeling_id" name="afdeling_id" required>
                        ...
                        @foreach($afdelings as $afdeling)
                            <option value="{{ $afdeling->id }}" {{ old('afdeling_id', $document->afdeling_id) == $afdeling->id ? 'selected' : '' }}>
                                {{ $afdeling->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('category_id') is-invalid @enderror" id="category_id" name="category_id" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" data-afdeling="{{ $category->afdeling_id }}" {{ old('category_id', $document->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <small class="form-text text-muted">
                        Format yang diperbolehkan: PDF, DOC, DOCX, XLS, XLSX. Maksimal ukuran: 10MB<br>
                        File saat ini: {{ basename($document->file_path) }}
                    </small>
                    @error('file')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
